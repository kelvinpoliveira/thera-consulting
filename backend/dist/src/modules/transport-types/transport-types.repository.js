var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@nestjs/common';
import { PrismaService } from '../../prisma/prisma.service.js';
let TransportTypesRepository = class TransportTypesRepository {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    findAll() {
        return this.prisma.transportType.findMany({ orderBy: { name: 'asc' } });
    }
    findById(id) {
        return this.prisma.transportType.findUnique({ where: { id } });
    }
    findByName(name) {
        return this.prisma.transportType.findUnique({ where: { name } });
    }
    create(dto) {
        return this.prisma.transportType.create({ data: dto });
    }
    update(id, dto) {
        return this.prisma.transportType.update({ where: { id }, data: dto });
    }
};
TransportTypesRepository = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [PrismaService])
], TransportTypesRepository);
export { TransportTypesRepository };
//# sourceMappingURL=transport-types.repository.js.map